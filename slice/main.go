...
fmt.Println(a)
只有对切片名、map名取地址、取值(不可以对切片和map类型的变量取值)以及传参 切片名之间、map名之间相互赋值不是对切片名、map名里保存的地址指向的堆里的空间进行操作 其他对切片名和map名的操作就是对切片名、map名里保存的地址指向的堆里的空间进行操作 不要考虑对切片名和map名进行取值操作的任何事
*/
func main() {
	fmt.Println("ok")
	/*
