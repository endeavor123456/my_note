...
//结构体是一种类型 可以叫他结构体、也可以叫他结构体类型
//结构体是值类型
//type关键字在函数外部使用
//使用type关键字时 更改的、新起的类型名不能与当前包已经存在的类型名重复
//结构体名小写只能在当前包里使用 大写则能在整个项目里使用(结构体名其实就是类型名)
//字段就是局部变量
//结构体名大写时，字段名首字母大写该字段才能被别的包使用,否则只能在当前包使用
/*
这个地方先这样理解，后期再改！！！！！！
...
/*
type 结构体名 struct{
	字段名1 字段类型  //字段就是局部变量，在结构体中字段名必须唯一
	字段名2 字段类型
}
*/
type Student struct {
