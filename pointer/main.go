...
func main() {
	/*
		...
		var a *int
		//当指针变量(注意是指针变量)的值为nil或0x0时，对该变量取值会报错 这时候需要使用到new() 这样就可以对该变量取值了
		fmt.Println(a)
		a = new(int)    //new函数【格式为：new(Type)】会开辟一个没有名字的Type类型的空间【他是一个变量，只不过没名字，但说的不是匿名变量】 并返回该空间的地址，地址为*Type类型
	...
	*/
	/*
		//指针切片[即为元素为指针类型的切片,所以只能对元素取值，要和切片类型的指针区分开]
		e := 10
		f := 20
		g := 30
		var slice = []*int{&e, &f, &g}
