...
func main() {
	...
	*/
	/*
		var arr1 [3]int
		arr1[0] = 23   //arr1[0]表示数组里下标为0的元素 数组的元素和变量的区别在于它不需要定义它是在定义数组变量时在数组变量内部又自动开辟的一块空间且用数组下标来表示这一块空间 切片的元素和变量的区别在于它是在切片指向的在堆中的空间的内部自动或手动开辟的一块空间且用切片下标来表示这一块空间  map理解起来比较麻烦需要单独看
		arr1[1] = 10
		arr1[2] = 10
		fmt.Println(arr1)
		fmt.Println("-----------------------------------")
